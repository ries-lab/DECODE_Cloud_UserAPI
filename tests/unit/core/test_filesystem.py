from tests.conftest import data_file1_name, data_file2_name, data_file1_contents, data_file2_contents, data_files, data_file1, \
...
class TestFilesystem:
    ...
    def test_list_directory(self, filesystem, data_files):
        ...
        assert FileInfo(data_file2_name, FileTypes.file, '{} Bytes'.format(len(data_file2_contents))) in files
        assert FileInfo(data_file1_name, FileTypes.file, '{} Bytes'.format(len(data_file1_contents))) in files

    def test_list_directory_subdir(self, filesystem, data_file1):
        files = list(filesystem.list_directory("data/test/"))
        assert len(files) == 1
        assert files[0] == FileInfo(data_file1_name, FileTypes.file,
    ...
    def test_get_file_info(self, filesystem, data_files):
        ...
        assert info == FileInfo(data_file1_name, FileTypes.file, '{} Bytes'.format(len(data_file1_contents)))

    def test_create_file(self, filesystem, cleanup_files):
        filesystem.create_file(data_file1_name, BytesIO(bytes(data_file1_contents, 'utf-8')))
